...
func TestCursorRoundTrip(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		createEntity(t, mux, tenant, map[string]any{"repository_id": "repo-1"})
	}
...
func TestMemoryListInvalidatesCache(t *testing.T) {
	...
	createEntity(t, mux, tenant, map[string]any{"repository_id": "r1"})
	...
	createEntity(t, mux, tenant, map[string]any{"repository_id": "r2"})
...
func TestUpdateAndDelete(t *testing.T) {
	_, mux := setupTestServer()
	resp := createEntity(t, mux, "t1", map[string]any{"repository_id": "r1"})
	id := resp["item"].(map[string]any)["id"].(string)
	body, _ := json.Marshal(map[string]any{"status": "completed", "overall_score": 85})
