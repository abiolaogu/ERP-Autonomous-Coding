...
func TestCursorRoundTrip(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		createEntity(t, mux, tenant, map[string]any{"name": "Repo " + string(rune('A'+i)), "url": "https://github.com/test/" + string(rune('a'+i)), "provider": "github"})
	}
	...
	if len(items1) != 2 {
		t.Fatalf("expected 2, got %d", len(items1))
	}
	...
	if len(items2) != 2 {
		t.Fatalf("expected 2, got %d", len(items2))
	}
	if items1[1].(map[string]any)["id"] == items2[0].(map[string]any)["id"] {
...
func TestMemoryListInvalidatesCache(t *testing.T) {
	...
	createEntity(t, mux, tenant, map[string]any{"name": "R1", "url": "https://gh.com/r1", "provider": "github"})
	...
	createEntity(t, mux, tenant, map[string]any{"name": "R2", "url": "https://gh.com/r2", "provider": "gitlab"})
	if _, ok := srv.cache.get(cacheKey); ok {
...
func TestUpdateAndDelete(t *testing.T) {
	_, mux := setupTestServer()
	resp := createEntity(t, mux, "t1", map[string]any{"name": "Del", "url": "https://gh.com/del", "provider": "github"})
